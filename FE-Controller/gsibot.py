...
def runCommands(cs_response, question, user):
    ...
    while commands != []:
        ...
        new_commands = []
        new_nl = ''
        if u"¬solrPublication" in command:
            new_commands, new_nl = solrPublication(command)
        elif u'¬solrResponsePublication' in command:
            current_response = sendChatScript(command, user)
            new_commands, new_nl = splitCommands(current_response)
        elif u'¬solrProject' in command:
            new_commands, new_nl = solrProject(command)
        elif u'¬solrResponseProject' in command:
            current_response = sendChatScript(command, user)
            new_commands, new_nl = splitCommands(current_response)
        elif u'¬solrPeople' in command:
            new_commands, new_nl = solrPeople(command)
        elif u'¬solrResponse' in command:
            current_response = sendChatScript(command, user)
            new_commands, new_nl = splitCommands(current_response)
        elif u'¬solrCount' in command:
            new_commands, new_nl = solrCount(command)
        elif u'¬solrcounted' in command:
            current_response = sendChatScript(command, user)
            new_commands, new_nl = splitCOmmands(current_response)
        ...
        # Add the results of the current command
...
def solrPublication(command):
    commands = []
    nl_response = ""

    return (commands, nl_response)

def solrProject(command):
    commands = []
    nl_response = ""

    return(commands, nl_response)

def solrPerson(command):
    commands = []
    nl_response = ""

    return (commands, nl_response)

def solrCount(command):
    commands = []
    nl_response = ""
